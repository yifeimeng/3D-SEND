function tagsInfo=readDMFilex86(signalFileName,logFileName,signalType)
% Y. Meng, June, 2014, University of Illinois
% This function is modified based on F. Sigworth's ReadDM3 (Auggust 2013). 
% This function supports spectrum, image and spectrum image.
% This function is modified to work on 32 bits matlab and therefore doesn't
% support dm4 file larger than 4GB.
% The signal type should be 'spec' (spectrum), 'img' (image) or 'si' (spectru
% m image). The returned tagsInfo is a structure contains all the important
% information. The origin of spectrum is automatically corrected to the
% true value.
%
% Original description by F. Sigworth
%-------------------------------------------
...
% Define tags for different types of signal. Any 1 dimension signal is
% treated as spectrum. Any 2 dimension signal is treated as image. Any 3 
% dimension signal is treated as spectrum image.
 spectrumTags={'ImageList 2 ImageData Calibrations Dimension 1 Scale'
 ...
 spectrumImageTags={'ImageList 2 ImageData Calibrations Dimension 1 Scale'
                    'ImageList 2 ImageData Calibrations Dimension 1 Units'
                    'ImageList 2 ImageData Calibrations Dimension 1 Origin'
                    ...
                    'ImageList 2 ImageData Calibrations Dimension 3 Units'
                    ...
                    'ImageList 2 ImageData Data'};
 imageTags={'ImageList 2 ImageData Calibrations Dimension 1 Scale'
            'ImageList 2 ImageData Calibrations Dimension 1 Units'
            'ImageList 2 ImageData Calibrations Dimension 1 Origin'
            'ImageList 2 ImageData Calibrations Dimension 2 Scale'
            'ImageList 2 ImageData Calibrations Dimension 2 Units'
            ...
            'ImageList 2 ImageData Dimensions 2'
            'ImageList 2 ImageData Data'};
 imageStackTags={'ImageList 2 ImageData Calibrations Dimension 1 Scale'
                 'ImageList 2 ImageData Calibrations Dimension 1 Units'
                 'ImageList 2 ImageData Calibrations Dimension 1 Origin'
                 'ImageList 2 ImageData Calibrations Dimension 2 Scale'
                 'ImageList 2 ImageData Calibrations Dimension 2 Units'
 ...
 switch signalType
     case 'img'
         tagsToSearch=imageTags;
     case 'spec'
         tagsToSearch=spectrumTags;
     case 'si'
         tagsToSearch=spectrumImageTags;
     case 'imgStack'
         tagsToSearch=imageStackTags;
 end



celltags=tagsToSearch;
...
signalFileID=fopen(signalFileName,'r');
...
p=uint32(1);  % byte pointer--also a global variable
...
% Output the important information about the signal

 switch signalType
     case 'spec'
         tagsInfo.energyScale=output{1};
         tagsInfo.energyUnit=char(output{2});
         tagsInfo.energyOrigin=-(output{3}*tagsInfo.energyScale);
         tagsInfo.energyDimSize=uint16(output{4});
         tagsInfo.pureData=output{5};

     case 'img'
         tagsInfo.naviXScale=output{1};
         tagsInfo.naviXUnit=char(output{2});
         tagsInfo.naviXOrigin=output{3};
         tagsInfo.naviYScale=output{4};
         tagsInfo.naviYUnit=char(output{5});
         tagsInfo.naviYOrigin=output{6};
         tagsInfo.naviXDimSize=uint16(output{7});
         tagsInfo.naviYDimSize=uint16(output{8});
         tagsInfo.pureData=reshape(output{9},tagsInfo.naviXDimSize,tagsInfo.naviYDimSize);
     case 'si'
         tagsInfo.naviXScale=output{1};
         tagsInfo.naviXUnit=char(output{2});
         tagsInfo.naviXOrigin=output{3};
         ...
         tagsInfo.naviYUnit=char(output{5});
         tagsInfo.naviYOrigin=output{6};
         tagsInfo.energyScale=output{7};
         tagsInfo.energyUnit=char(output{8});
         tagsInfo.energyOrigin=-(output{9}*tagsInfo.energyScale);
         tagsInfo.naviXDimSize=uint16(output{10});
         tagsInfo.naviYDimSize=uint16(output{11});
         tagsInfo.energyDimSize=uint16(output{12});
         tagsInfo.pureData=reshape(output{13},tagsInfo.naviXDimSize,tagsInfo.naviYDimSize,tagsInfo.energyDimSize);
     case 'imgStack'
         tagsInfo.naviXScale=output{1};
         tagsInfo.naviXUnit=char(output{2});
         tagsInfo.naviXOrigin=output{3};
         ...
         tagsInfo.naviYUnit=char(output{5});
         tagsInfo.naviYOrigin=output{6};
         tagsInfo.naviZScale=output{7};
         tagsInfo.naviZUnit=char(output{8});
         tagsInfo.naviZOrigin=output{9};
         tagsInfo.naviXDimSize=uint16(output{10});
         tagsInfo.naviYDimSize=uint16(output{11});
         tagsInfo.naviZDimSize=uint16(output{12});
         tagsInfo.pureData=reshape(output{13},tagsInfo.naviXDimSize,tagsInfo.naviYDimSize,tagsInfo.naviZDimSize);
 end
